...
theorem pow_eq {x y : ℝ} {n : ℕ} (Hxpos : 0 < x) (Hypos : 0 < y) (Hnpos : 0 < n) (Hxyn : x ^ n = y ^ n) : x = y :=
begin
...
noncomputable definition nth_root (x : ℝ) (n : ℕ) : ℝ := real.exp ((real.log x) / n)
...
theorem nth_root_pos {x n} : nth_root x n > 0 := real.exp_pos _

theorem nth_root_power {x : ℝ} {n} (Hxpos : 0 < x) (Hnpos : 0 < n): (nth_root x n) ^ n = x := 
by rw [nth_root, ←exp_mul, mul_div_cancel', real.exp_log Hxpos]; rw nat.cast_ne_zero; exact ne_of_gt Hnpos
...
begin
  intros Hf Hg ε Hε,
  have Hf1 := Hf (min ε 1) (lt_min Hε (by norm_num : 0 < (1 : ℝ))),
  have Hg1 := Hg (min ε 1) (lt_min Hε (by norm_num : 0 < (1 : ℝ))),
  cases Hf1 with δ₁ Hf2, cases Hf2 with Hδ₁ Hf3,
  cases Hg1 with δ₂ Hg2, cases Hg2 with Hδ₂ Hg3,
  existsi (min δ₁ δ₂), existsi (lt_min Hδ₁ Hδ₂),
  intros x hole Hx,
  have Hf4 := lt_min_iff.mp (Hf3 x hole ((lt_min_iff.mp Hx).1)),
  have Hg4 := lt_min_iff.mp (Hg3 x hole ((lt_min_iff.mp Hx).2)),
  sorry
end
...
begin
  ...
  split, 
  { apply lt_min, norm_num, apply half_pos (exp_pos _) },
  { intros x Hx, rw abs_pow,
    have Hx1 : abs x < 1, apply @lt_trans ℝ _ _ (1/2) _ (lt_min_iff.mp Hx).1, norm_num,
    have Hx2 : abs x < nth_root ε n := @lt_trans ℝ _ _ ((nth_root ε n)/2) _ (lt_min_iff.mp Hx).2 (half_lt_self nth_root_pos),
